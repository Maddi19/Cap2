...
library(here)
all<-read.csv(here("data","useful", "all_data.csv"))
villa <- all %>%
  subset(Site == "Doñana" & Bosque == "Villamanrique Chaparral" & Year == "2021" & Periodo=="2")
  
web <- table(villa$Planta, villa$Pollinator_id)
w1 <- networklevel(web)
netw_metrics <- list(w1)
...
sitems$tot.visits <- as.vector(sitems$tot.visits)
...
create_plot <- function(data, x_variable) {
  ...
  ggplot(data, aes_string(x = x_variable, y = "efecto_parcial")) +
    ...
    geom_smooth(method = "lm", color = "#d7191c", fill = "#fdae61", se = TRUE)  + 
    labs(
