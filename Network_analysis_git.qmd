...
plots_d <- map(variables, ~ create_plot_with_predictions(sitems.fs.d, .x, models.d.sub$m6))
...
plots.sub.sn <- map(variables, ~ create_plot.sn(sitems.fs.d.sub, .x))
...
plots.g.sn<- map(variables, ~ create_plot.sn(sitems.fs.g, .x))

for (p in plots.g) {
  print(p)
}

p.d.sn <- plots.sub.sn [[1]]
...
p.g.rs_final <- ggdraw() +
  draw_label("Gorbea", fontface = "bold", size = 14, x = 0.5, y = 1.08, hjust = 0.5) +
  draw_plot(p.g.rs, x = 0, y = 0, width = 1, height = 1)+
  theme(plot.margin = margin(35, 10, 10, 10)) 
