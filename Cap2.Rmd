...
For the models relating species-level indices to the reproductive success of each species, as specialization index gives us an idea of the number of different links a species could have, we removed the variable degree. In the case of Doñana, no variable significantly influenced the fruit set of the plants. However, in Gorbea, plant species richness (estimate = -1.24, z value = -2.35, p = 0.02; Figure 6A) and total received visits (estimate=-1.94, z=-2.6, p=0.008; Figure 6C) significantly reduced plant´s fruit set, while weighted closeness of each species (estimate=1.18, z value= 2.5, p=0.012; Figure 6B) and contribution to nestedness (estimate = 2.08, z value = 3.6, p value \< 0.0001; Figure 6D) had significant positive effects. Furthermore, we observed that sampling period significantly modulated the effects of contribution to nestedness and total received visits on fruit set, indicating that these relationships are not constant, but rather context-dependent (Supplementary Table S1 and Figure S16). Additionally, the interaction between sampling period and plant species richness exhibited a trending, non-significant effect (estimate = 0.214, z value = 1.763, p = 0.078), suggesting a possible temporal influence on the relationship between plant diversity and fruit set. Furthermore, the study revealed that reproductive success at the species level remained constant throughout the flowering season at both study sites (Supplementary Table S1). For the subset of plant species that we had at least data of 3 different periods, we could observed

```{r plots_splevel}
knitr::include_graphics("Figs/p.g_sp_final1.png")
```

In the case of the seed number each fruit produced, different trends were observed.
...
We found no evidence that variation in network nestedness (estimate=-0.0032, p=0.39) or variation in functional complementarity (estimate=0.0031, p=0.55) of pollinators was related to community level reproductive success. (quitando el dato de hinjos con nestedness altisimo)

In the Doñana study area, we found that the stability of plant reproductive success was influenced by pollinator richness and the total number of visits received. The findings indicated that pollinator richness led to a significant decrease in variability of reproductive success (estimate=-0.41, p\<0.0001), thereby enhancing stability. Concurrently, the number of total visits received resulted in an increase in variability of reproductive success (estimate=0.32, p=0.016), leading to a reduction in stability. In a similar manner, the plant richness (estimate=0.27, p=0.048) and flowering period (estimate=0.13, p\<0.001) positively influenced on the variability of reproductive success, thereby decreasing its stability. However, in Gorbea, none of these variables exerted a significant influence on reproductive success. Nevertheless, we observed discrepancies in the variability of reproductive success between 2021 and 2022, with the latter year demonstrating greater variability (estimate=0.39, p=0.0002).

```{r cv.fs, echo=FALSE} knitr::include_graphics("Figs/plots_cv_doñana.png")}
```
